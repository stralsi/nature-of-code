...
(function(ns){
  ...
  ns.arrows.drawForceOnMover = function(context,mover,force,color){
    ...
    //draw it
    ns.arrows.drawArrow(context,mover.location.x,mover.location.y,forceForDisplay.x,forceForDisplay.y,color);
  }

  //Drawing the velocity and acceleration is completely optional, just for me to understand what is happening.
  ns.arrows.drawVelocityAndAcceleration = function(context,mover){
      //draw velocity
      //make a copy just for display purposes, so that we don't affect the real velocity
      var velocityForDisplay = new ns.Vector2D(mover.velocity.x, mover.velocity.y);
      //scale it up so we can see it
      velocityForDisplay = velocityForDisplay.multiply(5);
      //translate it on top of the mover's body
      velocityForDisplay = velocityForDisplay.addVector(mover.location);
      //draw it
      ns.arrows.drawArrow(context,mover.location.x,mover.location.y,velocityForDisplay.x,velocityForDisplay.y,'blue');

      //draw acceleration
      //make a copy just for display purposes, so that we don't affect the real acceleration
      var accelerationForDisplay = new ns.Vector2D(mover.acceleration.x, mover.acceleration.y);
      ...
      ns.arrows.drawArrow(context,mover.location.x,mover.location.y,accelerationForDisplay.x,accelerationForDisplay.y,'red');
  };

  ns.arrows.drawArrow = function(context, sourceX, sourceY, destinationX, destinationY, color){
      context.beginPath();
